...
class TopCustomerInfo extends Component {
  ...
  componentDidMount() {
    ...
    if (!$.isEmptyObject(topCustomerInfo)) {
      //上位お客様追加でも修正したい場合
      document.getElementById("topCustomerNo").innerHTML =
        topCustomerInfo.topCustomerNo;
      $("#topCustomerName").val(topCustomerInfo.topCustomerName);
      $("#topCustomerAbbreviation").val(
        topCustomerInfo.topCustomerAbbreviation
      );
      ...
      if (
        topCustomerNo !== null &&
        topCustomerNo !== "" &&
        topCustomerNo !== undefined
      ) {
        var topCustomerMod = {};
        topCustomerMod["topCustomerNo"] = topCustomerNo;
        axios
          .post(this.state.serverIP + "topCustomerInfo/init", topCustomerMod)
          .then((resultMap) => {
            topCustomerMod = resultMap.data.topCustomerMod;
            document.getElementById("topCustomerNo").innerHTML =
              topCustomerMod.topCustomerNo;
            $("#topCustomerName").val(topCustomerMod.topCustomerName);
            $("#topCustomerAbbreviation").val(
              topCustomerMod.topCustomerAbbreviation
            );
  ...
  topCustomerToroku() {
    if (
      ...
      if (actionType === "update") {
        ...
        axios
          .post(this.state.serverIP + "topCustomerInfo/toroku", topCustomerInfo)
          .then((resultMap) => {
            if (resultMap.data) {
              this.setState({
                myToastShow: true,
            ...
            <Row>
              <Col className="text-center">
                {actionType === "update" ? (
                  <Button
                    size="sm"
                    onClick={this.topCustomerToroku.bind(this)}
                    variant="info"
                    id="update"
                    type="button"
                  >
