...
class DutyRegistration extends React.Component {
  ...
  tableValueChange = (event, cell, row) => {
    ...
    if (dataData[row.id]["hasWork"] === this.state.hasWork[0]) {
      ...
      if (row.startTimeHours === "") row.startTimeHours = "00";
      if (row.startTimeMinutes === "") row.startTimeMinutes = "00";
      if (row.endTimeHours === "") row.endTimeHours = "00";
      if (row.endTimeMinutes === "") row.endTimeMinutes = "00";
  ...
  tableValueChangeAfter = (event, cell, row) => {
    ...
    let startTime =
      (row.startTimeHours === undefined ? "00" : row.startTimeHours) +
      ":" +
      (row.startTimeMinutes === undefined ? "00" : row.startTimeMinutes);
    let endTime =
      (row.endTimeHours === undefined ? "00" : row.endTimeHours) +
      ":" +
      (row.endTimeMinutes === undefined ? "00" : row.endTimeMinutes);
    row.startTime = startTime;
    row.endTime = endTime;

    let workHour = this.getWorkHour(
      startTime,
      endTime,
      this.state.lunchBreakStartTime,
      this.state.lunchBreakFinshTime,
      this.state.lunchBreakTime,
      this.state.nightBreakStartTime,
      this.state.nightBreakfinshTime,
      this.state.nightBreakTime
    );
  ...
  getWorkHour = (
    ...
    if (startTime === "" || endTime === "") return workHour;

    let startTimeNum = Number(startTime.replace(":", ""));
    let endTimeNum = Number(endTime.replace(":", ""));

    if (nightBreakStartTime == "0000") nightBreakStartTime = "2500";
    if (nightBreakfinshTime == "0000") nightBreakfinshTime = "2500";

    if (
      startTimeNum <= lunchBreakStartTime &&
      endTimeNum <= lunchBreakStartTime
    )
      workHour = publicUtils.nullToEmpty(
        publicUtils.timeDiff(startTime, endTime)
      );
    else if (
      lunchBreakStartTime <= startTimeNum &&
      startTimeNum <= lunchBreakFinshTime &&
      lunchBreakStartTime <= endTimeNum &&
      endTimeNum <= lunchBreakFinshTime
    )
      workHour = "";
    else if (
      lunchBreakFinshTime <= startTimeNum &&
      startTimeNum <= nightBreakStartTime &&
      lunchBreakFinshTime <= endTimeNum &&
      endTimeNum <= nightBreakStartTime
    )
      workHour = publicUtils.nullToEmpty(
        publicUtils.timeDiff(startTime, endTime)
      );
    else if (
      nightBreakStartTime <= startTimeNum &&
      startTimeNum <= nightBreakfinshTime &&
      nightBreakStartTime <= endTimeNum &&
      endTimeNum <= nightBreakfinshTime
    )
      workHour = "";
    else if (
      startTimeNum >= nightBreakfinshTime &&
      ...
      workHour =
        publicUtils.nullToEmpty(publicUtils.timeDiff(startTime, endTime)) -
        lunchBreakTime;
    else if (
      startTimeNum <= lunchBreakStartTime &&
      nightBreakStartTime <= endTimeNum &&
  ...
  setTableStyle = (rowId, day) => {
    ...
    let target = $("#dutyDataRowNumber-" + rowId)
      .parent()
      .parent()[0];
    DutyRegistrationJs.removeRowAllClass(target);
    for (let i = 0; i < dateData.length; i++) {
      target = $("#dutyDataRowNumber-" + dateData[i].id)
        .parent()
        .parent()[0];
          ...
          if (day !== undefined && day !== null && dateData[i].day === day) {
            if (dateData[i].hasWork == this.state.hasWork[1]) {
              if (dateData[i].isWork == 0)
                DutyRegistrationJs.addRowClass(
                  target,
                  "dutyRegistration-SleepSelect"
                );
              else
                DutyRegistrationJs.addRowClass(
                  target,
                  "dutyRegistration-WorkSelect"
                );
              ...
              if (dateData[i].isWork == 0)
                ...
                DutyRegistrationJs.addRowClass(
                  target,
                  "dutyRegistration-WorkSleepSelect"
                );
            }
          } else {
            if (dateData[i].hasWork == this.state.hasWork[1]) {
              if (dateData[i].isWork == 0)
                DutyRegistrationJs.addRowClass(
                  target,
                  "dutyRegistration-SleepConfirmation"
                );
              else
                DutyRegistrationJs.addRowClass(
                  target,
                  "dutyRegistration-WorkConfirmation"
                );
            } else if (dateData[i].hasWork == this.state.hasWork[0]) {
              if (dateData[i].isWork == 0)
                DutyRegistrationJs.addRowClass(
                ...
                DutyRegistrationJs.addRowClass(
                  target,
                  "dutyRegistration-WorkSleepConfirmation"
                );
            }
          }
        } else {
  ...
  getWorkData = () => {
      ...
      .then((resultMap) => {
        if (resultMap.data.breakTime !== null) {
          ...
          for (let i = 0; i < defaultDateData.length; i++) {
            ...
            dateData[dayIndex].hasWork =
              this.state.hasWork[defaultDateData[i].isWork];
            if (defaultDateData[i].isWork == 1) {
              dateData[dayIndex].startTime = publicUtils.timeInsertChar(
                publicUtils.nullToEmpty(defaultDateData[i].startTime)
              );
              dateData[dayIndex].startTimeHours =
                publicUtils.nullToEmpty(defaultDateData[i].startTime) === ""
                  ? ""
                  : defaultDateData[i].startTime.substring(0, 2);
              dateData[dayIndex].startTimeMinutes =
                publicUtils.nullToEmpty(defaultDateData[i].startTime) === ""
                  ? ""
                  : defaultDateData[i].startTime.substring(2, 4);
              dateData[dayIndex].endTime = publicUtils.timeInsertChar(
                publicUtils.nullToEmpty(defaultDateData[i].endTime)
              );
              dateData[dayIndex].endTimeHours =
                publicUtils.nullToEmpty(defaultDateData[i].endTime) === ""
                  ? ""
                  : defaultDateData[i].endTime.substring(0, 2);
              dateData[dayIndex].endTimeMinutes =
                publicUtils.nullToEmpty(defaultDateData[i].endTime) === ""
                  ? ""
                  : defaultDateData[i].endTime.substring(2, 4);
              dateData[dayIndex].sleepHour = sleepHour;
              let startTime = publicUtils.timeInsertChar(
                publicUtils.nullToEmpty(defaultDateData[i].startTime)
              );
              let endTime = publicUtils.timeInsertChar(
                publicUtils.nullToEmpty(defaultDateData[i].endTime)
              );
              dateData[dayIndex].workHour = this.getWorkHour(
                startTime,
                endTime,
                lunchBreakStartTime,
                lunchBreakFinshTime,
              ...
              dateData[dayIndex]["workContent"] = publicUtils.nullToEmpty(
                defaultDateData[i].workContent
              );
              dateData[dayIndex]["remark"] = publicUtils.nullToEmpty(
                defaultDateData[i].remark
              );
            } else {
              dateData[dayIndex].sleepHour = "";
            }
            dateData[dayIndex].confirmFlag =
              publicUtils.nullToEmpty(defaultDateData[i].confirmFlag) === ""
                ? ""
                : defaultDateData[i].confirmFlag;
            dateData[dayIndex].errorFlag = "";
          }
          this.setState({
            breakTime: resultMap.data.breakTime,
            dateData: dateData,
            workDays: workDays,
            workHours: workHours,
            employeeNo: resultMap.data.employeeNo,
            siteCustomer: resultMap.data.siteCustomer,
            customer: resultMap.data.customer,
            siteResponsiblePerson: resultMap.data.siteResponsiblePerson,
            systemName: resultMap.data.systemName,
    ...
    if (!this.state.disabledFlag) {
      axios
        .post(
          this.state.serverIP + "dutyRegistration/selectWorkRepot",
          postData
        )
  ...
  onSubmit = (event) => {
    ...
    for (let i = 0; i < dataInfo["dateData"].length; i++) {
      dataInfo["dateData"][i]["isWork"] =
        dataInfo["dateData"][i]["hasWork"] == this.state.hasWork[0] ? 0 : 1;
    ...
    if (actionType === "insert") {
      axios
        .post(this.state.serverIP + "dutyRegistration/dutyInsert", dataInfo)
        .then((resultMap) => {
        ...
        for (let i = 0; i < dateData.length; i++) {
          ...
          if (dateData[i]["hasWork"] == this.state.hasWork[1]) {
            if (publicUtils.isEmpty(dateData[i]["startTime"])) {
              errorMessage += messageUtils.getMessage(
                "E0003",
                dateData[i]["day"],
                true
              );
            ...
            if (
              publicUtils.isEmpty(dateData[i]["workHour"]) ||
              Number(dateData[i]["workHour"]) <= 0
            ) {
              errorMessage += messageUtils.getMessage(
                "E0005",
                dateData[i]["day"],
          ...
          if (
            !publicUtils.isEmpty(dateData[i]["startTime"]) &&
            !publicUtils.isEmpty(dateData[i]["endTime"])
          ) {
  ...
  hasWorkFormatter = (cell, row) => {
    let returnItem = <span id={"dutyDataRowNumber-" + row.id}>{cell}</span>;
    if (
      ...
      returnItem = (
        <span
          id={"dutyDataRowNumber-" + row.id}
          class="dutyRegistration-DataTableEditingCell"
        >
          <select
            class=" form-control editor edit-select"
            name="hasWork"
            value={cell}
            onChange={(event) => {
  ...
  startTimeFormatter = (cell, row) => {
    ...
    if (
      ...
      !this.state.disabledFlag &&
      row.confirmFlag !== "1" &&
      this.state.dateData[row.id].hasWork === this.state.hasWork[1]
    ) {
      // returnItem = <span
          ...
          <Row style={{ padding: "0px", margin: "0px" }}>
            <Col style={{ padding: "0px", margin: "0px" }}>
              <span
                ...
                class={
                  row.errorFlag === "Time"
                    ? "dutyRegistration-DataTableEditingCellError"
                    : "dutyRegistration-DataTableEditingCell"
                }
              >
                <select
                  class=" form-control editor edit-select"
                  name="startTimeHours"
                  value={row.startTimeHours}
                  onChange={(event) => {
                    this.tableValueChange(event, cell, row);
                    this.tableValueChangeAfter(event, cell, row);
                  }}
  ...
  workHourFormatter = (cell) => {
    if (Number(cell) > 0) return cell + "H";
  };
  ...
  sleepHourFormatter = (cell, row) => {
    ...
    if (
      ...
      !this.state.disabledFlag &&
      this.state.dateData[row.id].hasWork === this.state.hasWork[1] &&
      this.state.breakTime.breakTimeFixedStatus == 0
    ) {
      returnItem = (
      ...
      !this.state.disabledFlag &&
      this.state.dateData[row.id].hasWork === this.state.hasWork[1] &&
      this.state.breakTime.breakTimeFixedStatus
    ) {
  ...
  beferShuseiTo = (actionType) => {
    ...
    for (let i in this.state.dateData) {
      if (
        this.state.dateData[i].workContent !== null ||
        this.state.dateData[i].workContent !== ""
      ) {
        flag = true;
        break;
      }
  ...
  handleRowSelect = (row, isSelected, e) => {
    if (row.confirmFlag === "1") {
      if (isSelected) {
        ...
        if (index !== -1) rowNo.splice(index, 1);
        this.setState({
          rowNo: rowNo,
        });
        this.setTableStyle(row.id);
  ...
  setYearMonth = (date) => {
    ...
    if (temp === now) {
      ...
      axios
        .post(this.state.serverIP + "dutyRegistration/getFlag", postData)
        .then((response) => {
      ...
      <div className={isMobileDevice ? "clear-grid-padding" : ""}>
        ...
        <div>
          ...
          <Modal
            ...
            centered
            backdrop="static"
            onHide={this.handleHideModal.bind(this, "breakTime")}
            show={this.state.showbreakTimeModal}
            dialogClassName="modal-breakTime"
          >
            ...
            <Row className="align-items-center">
              ...
              <Col sm={3} md={{ span: 3, offset: 1 }} hidden>
                <InputGroup size="sm" className="mb-3">
                  ...
                  <input
                    style={{ width: "150px" }}
                    value={this.state.siteResponsiblePerson}
                    name="siteResponsiblePerson"
                    onChange={this.valueChange}
                    className="inputWithoutBorder"
                  />
                </InputGroup>
            ...
            <Row>
              <Col sm={12}>
                ...
                >
                  ...
                  <TableHeaderColumn
                    hidden
                    dataField="startTimeHours"
                    dataFormat={this.startTimeHoursFormatter}
                  >
                    開始時
                  </TableHeaderColumn>
                  <TableHeaderColumn
                    hidden
                    dataField="startTimeMinutes"
                    dataFormat={this.startTimeMinutesFormatter}
                  >
                    開始分
                  </TableHeaderColumn>
                  ...
                  <TableHeaderColumn
                    hidden
                    dataField="endTimeHours"
                    dataFormat={this.endTimeHoursFormatter}
                  >
                    終了時
                  </TableHeaderColumn>
                  <TableHeaderColumn
                    hidden
                    dataField="endTimeMinutes"
                    dataFormat={this.endTimeMinutesFormatter}
                  >
                    終了分
                  </TableHeaderColumn>
                  ...
                  <TableHeaderColumn
                    ...
                    width="70"
                    dataField="sleepHour"
                    dataFormat={this.sleepHourFormatter}
                    hidden
                  >
                    休憩時間
                  </TableHeaderColumn>
