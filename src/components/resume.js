...
class resume extends React.Component {
  ...
  searchResume = () => {
    axios
      .post(this.state.serverIP + "resume/selectResume")
  ...
  searchEmployeeName = () => {
    axios
      .post(this.state.serverIP + "resume/selectEmployeeName")
      .then((response) => {
        this.setState({
  ...
  InsertResume = (e) => {
    ...
    if (
      !(
        this.state.employeeList[0].resumeInfo === undefined ||
        this.state.employeeList[0].resumeInfo === null ||
        this.state.employeeList[0].resumeInfo === ""
      )
    ) {
      if (
        this.state.employeeList[0].resumeName === undefined ||
        this.state.employeeList[0].resumeName === null ||
        this.state.employeeList[0].resumeName === ""
      ) {
        this.setState({
          errorsMessageShow: true,
          method: "put",
    ...
    if (
      !(
        this.state.employeeList[1].resumeInfo === undefined ||
        this.state.employeeList[1].resumeInfo === null ||
        this.state.employeeList[1].resumeInfo === ""
      )
    ) {
      if (
        this.state.employeeList[1].resumeName === undefined ||
        this.state.employeeList[1].resumeName === null ||
        this.state.employeeList[1].resumeName === ""
      ) {
        this.setState({
          errorsMessageShow: true,
          method: "put",
    ...
    formData.append(
      "filePath1",
      publicUtils.nullToEmpty($("#filePath1").get(0).files[0])
    );
    formData.append(
      "filePath2",
      publicUtils.nullToEmpty($("#filePath2").get(0).files[0])
    );
    axios
  ...
  downloadResume = (resumeInfo, no, resumeName) => {
    let fileKey = "";
    let downLoadPath = "";
    if (resumeInfo !== null && resumeInfo.split("file/").length > 1) {
      ...
      downLoadPath = (
        ...
        resumeName.replace(this.state.employeeName + "_", "") +
        "." +
  ...
  setUpButton = (cell, row) => {
    return (
      <form id={"UpButtonForm" + row.rowNo}>
        <Button size="sm" onClick={(event) => this.addFile(event, row.rowNo)}>
          <FontAwesomeIcon />
          {row.haveFile !== true ? " 添付" : " 添付済み"}{" "}
        </Button>
        <Form.File
          id={"filePath" + row.rowNo}
          hidden
          onChange={(event) => this.changeFile(event, row)}
        />
      </form>
    );
  };
            ...
            <Col sm={12}>
              ...
              >
                ...
                <TableHeaderColumn
                  ...
                  tdStyle={{ padding: ".45em" }}
                  dataField="nodata"
                  dataFormat={this.setUpButton.bind(this)}
                >
                  添付状況
                </TableHeaderColumn>
                <TableHeaderColumn
                ...
                <TableHeaderColumn
                  width="25%"
                  tdStyle={{ padding: ".45em" }}
                  dataField="resumeName"
                  dataFormat={this.getResumeName}
                >
                  履歴書名
                </TableHeaderColumn>
                <TableHeaderColumn
                ...
                <TableHeaderColumn
                  ...
                  dataFormat={this.setUpDate.bind(this)}
                >
                  更新日
                </TableHeaderColumn>
              </BootstrapTable>
